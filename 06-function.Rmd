...
练习：利用不定数量参数，写一个函数`my_sum()`，可以求任意个变量的和。
...
print(my_sum(3,4,5))   # 12
```
<iframe width="800" height="500" frameborder="0" src="https://pythontutor.com/iframe-embed.html#code=def%20my_sum%28*numbers%29%3A%0A%20%20total%20%3D%200%0A%20%20for%20i%20in%20numbers%3A%0A%20%20%20%20total%20%2B%3D%20i%20%0A%20%20return%20total%0A%20%20%0Aprint%28my_sum%281,2,3,4%29%29&codeDivHeight=400&codeDivWidth=350&cumulative=false&curInstr=0&heapPrimitives=nevernest&origin=opt-frontend.js&py=3&rawInputLstJSON=%5B%5D&textReferences=false"> </iframe>

### 可变参数以dict形式传递

参数前加1个\*，可变参数就会组成一个元组tuple，进入函数。2个\*\*，就会以字典dict进入函数
...
在我们使用`map`、`filter`，或者列表推导的时候，如果要对列表做的操作**非常简单、一目了然**，并且可以**写进一行**（比如乘以二，或者取最后一个字母等等），那么我们可以不用专门写一个函数，而直接放一个匿名函数进去。
...
例如，我们对一个列表`a = [1,2,3,5,7,9]`，`reduce`一下应用我们前面写的`add()`函数。
...
`square`函数，相当于一个**附带了`n=2`的`fun()`函数**

1. `power(n = 2)`，其中`n = 2`
2. 在`power()`函数的内部，`fun(x)`是可以看见这个`n=2`的
3. `return fun`的时候，会把`n=2`，一并返回到外层
4. `square = power(2) `,此时我们把`square`这个名字，绑定给`fun`这个函数（闭包），而后者同时还附带了`n=2`



前面讲变量作用域，python找一个名称，是从内到外

1. L局部作用域：即函数体内。例如本例中`fun`函数的`x`
2. Enclosing function locals：函数中嵌套函数的外部：例如本例中`fun`函数的`n`
3. G全局：代码的最外层
