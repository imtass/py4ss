...
判断是否同时在刮风和下雨
```{python}
print(is_blowing and is_raining)
```
...
```{python eval=FALSE}
bad_weather = bool_and(is_blowing, is_blowing) # 这是随便写的，无法执行
```
...
所以，你在`if`后面跟一个布尔型变量`bad_weather`，或者一个条件`is_blowing and is_blowing`，是等价的。
