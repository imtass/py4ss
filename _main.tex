...
本文是Python入门和数据处理教程，底稿为经管类本科课堂教学用的讲义，以供同学们自学和参考。
...
一般情况下，所谓一个Python程序，仅仅是一个你电脑里的\textbf{纯文本文件}，但扩展名是*.py，这个文件本质上一个.txt文件没什么不同，都可以用任何文本编辑器（例如你电脑里的``记事本''）打开和编辑。

我们要做的工作，就是用一个文本编辑器，编辑一个.py文件，然后把你要的代码写进去，用不同的方法去执行这个文件里代码。可能是在系统里一次性执行整个文件里所有代码，也可能是在一个交互环境里一步一步地执行。
